...
function reducer(state,action){
  if (state.inputData.length >= ACTIONS.MAX_DIGITS || state.displayData.length >= ACTIONS.MAX_DIGITS) {
    return {
      ...state,
...
function App() {
  ...
  return (
    <div className='main'>
      <div className='box'>
      <div id='display'>{state.displayData}{state.finalResult}</div>
      <div id='input' >{state.inputData}</div>
      <div className='btns'>
      <button id='clear'onClick={()=>dispatch({type: ACTIONS.CLEAR_DIGIT,})}>AC</button>
      <button id='divide' onClick={()=>dispatch({type: ACTIONS.OPR_DIGIT, payload: '/'})}>/</button>
      <button id='multiply' onClick={()=>dispatch({type: ACTIONS.OPR_DIGIT, payload: '*'})}>x</button><br/>
      ...
      <button id='subtract' onClick={()=>dispatch({type: ACTIONS.OPR_DIGIT, payload: '-'})} >-</button><br/>
      ...
      <button id='add' onClick={()=>dispatch({type: ACTIONS.OPR_DIGIT, payload: '+'})}>+</button><br/>
      ...
      <button id='three' onClick={()=>dispatch({type: ACTIONS.ADD_DIGIT, payload: '3'})}>3</button><br/>
      <button id='zero' onClick={()=>dispatch({type: ACTIONS.ADD_DIGIT, payload: '0'})}>0</button>
      <button id='decimal' onClick={()=>dispatch({type: ACTIONS.ADD_DECIMAL, payload: '.'})}>.</button>
